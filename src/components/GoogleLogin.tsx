...
export default function GoogleLogin({ role }: GoogleLoginProps) {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        // Note: No need to navigate here as login function handles it
      } catch (error) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-lg bg-blue-600 hover:bg-blue-700 transition-all text-white shadow-sm"
    >
